 ...
 * Description:Cryptocurrency Price Ticker Widget displays current prices of crypto coins - bitcoin, litecoin, ethereum, ripple, dash etc.
...
define( 'Crypto_Currency_Price_Widget_VERSION', '1.1' );
...
final class Crypto_Currency_Price_Widget {
	...
	wp_enqueue_script('ccpw_marque_js',Crypto_Currency_Price_Widget_URL. 'assets/marquee/jquery.webticker.min.js', null, null, 'all');
	 wp_add_inline_script('ccpw_marque_js', 'jQuery(document).ready(function($){
	 	...
	 	$(".ccpw-sticky-ticker ul").webTicker({
	 				speed:'.$t_speed.',
	 				moving:true,
	 				 duplicate:true,
	 				 hoverpause:true,
	...
	$all_coin_data=$this->get_api_data(10);
		...
		if( ! empty( $all_coin_data ) && is_array($all_coin_data)) {
			...
			$output .= '<div class="ccpw-container  '.$container_cls.'"> 
	       <div class="'.$cls.'"  ><ul  id="'.$id.'">';
			$output .= $crypto_html;
			$output .= '</ul></div></div>';
		...
		function generate_html($coin,$type,$display_logo,$display_changes,$ticker_position,$ticker_speed,$back_color,$font_color,$custom_css){
			$coin_html='';
				...
				$coin_html.=  '<li id="'.esc_attr( $coin_id).'">';
						...
						$coin_html.=$change_sign.$percent_change_1h ;
        ...
        if( empty( $coinslist ) ) {
			...
			if( ! empty( $coinslist ) ) {
			 set_transient( 'ccpw-coins', $coinslist, $expireTime );
			}
		 }
